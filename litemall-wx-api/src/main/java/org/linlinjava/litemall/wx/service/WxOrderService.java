...
public class WxOrderService {
    ...
    public Object submit(Integer userId, String body) {
        ...
        Integer deliveryMethod = JacksonUtil.parseInteger(body, "deliveryMethod");
        ...
        // 若配送方式为自提,不设置运费
        if (deliveryMethod == null || deliveryMethod != 2) {
     ...
     * @return 支付订单结果
     */
    public Object balancePay(Integer userId, String body, HttpServletRequest request) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer orderId = JacksonUtil.parseInteger(body, "orderId");
        ...
        if (actualPrice.compareTo(userBalance) > 0) {
            return ResponseUtil.fail(ORDER_PAY_FAIL, "余额不足");
        }
        if (userService.updateUserBalanceById(userId, userBalance.subtract(actualPrice)) == 0) {
            return ResponseUtil.fail(ORDER_PAY_FAIL, "余额扣减失败");
        }

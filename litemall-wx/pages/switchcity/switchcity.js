...
Page({
  ...
  backHome: function () {
    let that = this;
    let periodText = '';
    let code = that.data.currentCityCode;
    if (code.length>0&&code.startsWith("3701")) {
      periodText = '今日达'
    } else {
      periodText = '全国送'
    }

    appInstance.globalData.defaultPeriodText = periodText;

    wx.setStorageSync('defaultPeriodText', periodText);
    wx.setStorageSync('localAddressText', that.data.county);

    var pages = getCurrentPages();
    var prevPage = pages[pages.length - 2]; //上一个页面 主页
    prevPage.setData({
      localAddressText: that.data.county,
